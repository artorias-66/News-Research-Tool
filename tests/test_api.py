...
Uses Starlette's synchronous TestClient (bundled with FastAPI) to avoid
the need for pytest-asyncio in the CI/CD pipeline.
"""
...
class TestMetricsEndpoint:
    ...
    def test_has_cache_metrics(self):
        client = make_client()
        response = client.get("/api/metrics")
        data = response.json()
...
class TestQueryEndpoint:
    ...
    def test_no_index_returns_400(self):
    ...
    def test_valid_query_with_mock(self):
        mock_vs = MagicMock()
        mock_doc = MagicMock()
